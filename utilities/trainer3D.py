...
from utilities.gaussianBlock3D import GaussianDiffusion3D
from utilities.model3D import Denoiser
import numpy as np

#%% Step-2: Denoiser and Image Encoder
''' 
This module is used for training LDSeg for 3D knee images
...
class LDSeg3D(keras.Model):
    def __init__(self, network, ema_network, imgEncoder, encoder, decoder, 
        ...
        # model paths
        self.encoderPath = os.path.join(self.filepath[0], 'labelEncoder3D.hdf5')
        self.decoderPath = os.path.join(self.filepath[0], 'labelDecoder3D.hdf5')
        self.imgEncoderPath = os.path.join(self.filepath[0], 'imageEncoder3D.hdf5')
        self.denoiserPath = os.path.join(self.filepath[0], 'denoiser3D.hdf5')
...
def train_models(images, labels, 
    ...
    # Gaussian block
    gdf_util = GaussianDiffusion3D(beta_start=minbeta, 
                                   beta_end=maxbeta, 
                                   timesteps=total_timesteps, 
                                   schedule='cosine')                                 
    
    # model framework
    model = LDSeg3D(network = ema,

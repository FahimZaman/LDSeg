...
In the inference phase, image embedding is obtained first using the image encoder, and then the Denoiser is iterated for $t = T \cdots 1$ to obtain ![Math](https://latex.codecogs.com/png.latex?\tilde{z}_{l(0)}), staring with ![Math](https://latex.codecogs.com/png.latex?\tilde{z}_{l(T)}) as a Gaussian random noise ![Math](https://latex.codecogs.com/png.latex?\mathcal{N}(\mathrm{0, I})). Finally, the trained label decoder is used to get the final segmentation $\hat{y}$.
